<template>
    <div>
        ...
        <SectionHeader :title="__('admin.import_questions_for', { title: exam.title })" />
        <p class="mb-4">{{ __('admin.import_preview_instructions') }}</p>

        <Alert v-if="hasErrors" type="error" :message="__('messages.import_preview_error')" />
        <Alert v-else type="success" :message="__('messages.import_preview_success')" />
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-100 text-start dark:bg-gray-700">
                            ...
                            <th class="p-2">{{ __('labels.question_text') }}</th>
                        ...
                        <tr v-for="row in preview" :key="row.row" class="border-t dark:border-gray-700">
                            ...
                            <td class="p-2">
                                <ul class="list-inside list-disc">
                                    <li v-for="(option, index) in row.options" :key="index">{{ option }}</li>
                                </ul>
                            </td>
        ...
        <div class="mt-6 flex justify-end gap-4">
            <BaseButton as="a" :href="route('admin.exams.show', exam.id)" class="bg-gray-500 hover:bg-gray-600">{{ __('common.cancel') }}</BaseButton>
...
const submit = () => {
    form.post(route('admin.exams.questions.import.commit', { exam: props.exam.id }));
};
</script>
