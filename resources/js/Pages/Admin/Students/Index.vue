    ...
    <div>
        ...
        <SectionHeader :title="__('admin.students_list')">
            <template #action>
                <div class="flex gap-2">
                    <BaseButton @click="exportStudents" class="bg-gray-700 hover:bg-gray-800"> {{ __('buttons.export_list') }} </BaseButton>
                    <BaseButton as="a" :href="route('admin.students.create')">{{ __('buttons.add_student') }}</BaseButton>
                    <BaseButton as="a" :href="route('admin.students.import.form')" class="bg-green-600 hover:bg-green-700">
        ...
        <Card class="mb-4">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <BaseInput v-model="filters.q" :placeholder="__('common.search_name_or_email')" />
                ...
                <BaseSelect v-model="filters.is_approved">
                    <option value="">{{ __('common.any_approval_status') }}</option>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-100 text-start dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-4 py-2">{{ __('common.actions') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="student in students.data" :key="student.id" class="border-t border-gray-200 dark:border-gray-700">
                            <td class="px-4 py-2 whitespace-nowrap">{{ student.name }}</td>
                            <td class="px-4 py-2">{{ student.email }}</td>
                            <td class="px-4 py-2">{{ student.level?.name || 'N/A' }}</td>
                            <td class="px-4 py-2">
                            ...
                            <td class="space-x-2 px-4 py-2 whitespace-nowrap">
                                ...
                                <BaseButton @click="toggleApproval(student)">{{
                                    student.is_approved ? __('common.deny') : __('common.approve')
                                }}</BaseButton>
                                <BaseButton class="bg-red-500 hover:bg-red-600" @click="confirmDelete(student)">{{ __('common.delete') }}</BaseButton>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <ConfirmDialog
            ...
            :message="__('messages.delete_student_confirm', { name: studentToDelete?.name })"
...
const deleteStudent = () => {
    if (studentToDelete.value) {
        router.delete(route('admin.students.destroy', { student: studentToDelete.value.id }), {
            onFinish: () => {
