...
export function required(fieldName = "Field") {
	return (value) => (value || value === 0 ? true : `${fieldName} is required`);
}
...
export function minLength(n, fieldName = "Field") {
	return (value) =>
		value && value.toString().length >= n
			? true
			: `${fieldName} must be at least ${n} characters`;
}

export function isEmail(value) {
	const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
	return re.test(String(value).toLowerCase()) ? true : "Enter a valid email";
}
